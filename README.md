# 🛡️ Privacy-Preserving Federated Learning over Noisy AIoT Links 📡
...
We present a unified FL system that jointly addresses noisy links and privacy through three integrated mechanisms: 
1. 🔗 Per-client independent Markov channel models that capture realistic wireless dynamics including burst errors
2. 🎛️ A dynamic privacy budget scheduler with three adaptation factors — channel state, training phase, and budget urgency — that allocates DP noise based on real-time conditions
3. 🔐 Link-aware secure aggregation toggling that activates cryptographic protections only when channel quality supports the additional overhead
...
Third, **⏱️ latency**. Centralized training introduces round-trip delays between data generation at the edge and model deployment back to devices. For time-sensitive applications such as fall detection or anomaly monitoring, these delays can be unacceptable.
...
2. **🎛️ A dynamic privacy budget scheduler** with three multiplicative adaptation factors (channel state, training phase, budget urgency) that allocates the differential privacy noise level per round based on real-time link conditions and training dynamics.
...
**Round structure.** In each communication round $t = 1, 2, \ldots, T$:
...
3. Each selected client $k \in S_t$ initializes its local model with $w_t$ and performs $E$ epochs of stochastic gradient descent (SGD) on its local dataset $D_k$, producing updated parameters $w_k^{t+1}$.
4. Each client uploads $w_k^{t+1}$ (or the delta $\Delta w_k = w_k^{t+1} - w_t$) to the server.
...
**Quantifying non-IID with Dirichlet partitioning.** We simulate realistic data heterogeneity using the Dirichlet distribution. For each client $k$ and class $c$, we sample a probability vector $p_k \sim \text{Dir}(\alpha \cdot 1_C)$, where $\alpha > 0$ is the concentration parameter and $C$ is the number of classes. Each client then receives a proportion of each class's data according to $p_k$.

The parameter $\alpha$ controls the degree of heterogeneity:
- $\alpha \to \infty$: all clients have identical class proportions (IID)
- $\alpha = 1.0$: moderate heterogeneity
- $\alpha = 0.5$: significant skew — some clients see mostly one class (our setting)
- $\alpha \to 0$: extreme case — each client has data from only one class

In our experiments with $\alpha = 0.5$, a typical partition might give Client 1 predominantly walking data (45%), Client 2 mostly sitting data (60%), and Client 3 mostly stairs data (55%), with small amounts of other classes distributed across clients.
...
**Definition.** A randomized mechanism $\mathcal{M}: \mathcal{D} \to \mathcal{R}$ is $(\epsilon, \delta)$-differentially private if for all datasets $D, D'$ that differ in exactly one record, and for all subsets $S \subseteq \mathcal{R}$ of possible outputs:

$$\Pr[\mathcal{M}(D) \in S] \leq e^{\epsilon} \cdot \Pr[\mathcal{M}(D') \in S] + \delta$$

**Interpreting the parameters:**

- **$\epsilon$ (epsilon), the privacy budget:** Controls the strength of the guarantee. Smaller $\epsilon$ means stronger privacy. At $\epsilon = 0$, the mechanism reveals nothing (outputs are identical regardless of any individual's data). At large $\epsilon$, the mechanism provides weak protection. Typical values in deployed systems range from $\epsilon = 1$ (strong, used by Apple) to $\epsilon = 10$ (moderate, common in ML research).

- **$\delta$ (delta), the failure probability:** A small probability (typically $10^{-4}$ to $10^{-6}$) that the $\epsilon$-bound may be violated. This allows for rare, low-probability events where more information leaks than $\epsilon$ would suggest. Setting $\delta < 1/n$ (where $n$ is the dataset size) ensures the guarantee is meaningful.
...
$$\bar{g} = \frac{1}{B} \sum_i \tilde{g}_i + \mathcal{N}(0, \sigma^2 C^2 I)$$

where $B$ is the batch size, $\sigma$ is the noise multiplier (controls the amount of noise), $C$ is the clipping norm, and $I$ is the identity matrix. The noise is drawn from a multivariate Gaussian distribution with zero mean and variance $\sigma^2 C^2$ in each coordinate.

**The privacy-utility tradeoff.** Larger $\sigma$ means more noise, stronger privacy (smaller effective $\epsilon$ per step), but worse model accuracy because the true gradient signal is obscured. Smaller $\sigma$ means less noise, better accuracy, but weaker privacy (larger effective $\epsilon$). Choosing $\sigma$ is the central design decision in DP-SGD.

#### 2.3.3 Privacy Composition and the Budget Metaphor

A critical property of DP is **composition**: applying multiple DP mechanisms to the same data accumulates privacy loss. In FL, each round of training constitutes a separate DP mechanism applied to the clients' data. After $T$ rounds, the total privacy loss is not $\epsilon$ per round but a composed quantity that grows with $T$.

**The budget metaphor.** Think of the total privacy budget $\epsilon_{\text{total}}$ as a bank account. Each round of training withdraws some amount. Once the account reaches zero, no more training is allowed — continuing would violate the stated privacy guarantee. The challenge is to spend this budget wisely: allocate enough per round for useful learning, but not so much that the budget runs out prematurely.

**Naive composition** sums individual $\epsilon$ values: $\epsilon_{\text{total}} = \sum_t \epsilon_t$. This is simple but loose — it overestimates the actual privacy loss, meaning you stop training earlier than necessary.
...
Rényi Differential Privacy [13] provides tighter privacy accounting by tracking privacy loss at multiple "orders" simultaneously. The Rényi divergence of order $\alpha$ between two distributions $P$ and $Q$ is:

$$D_\alpha(P \| Q) = \frac{1}{\alpha - 1} \log \mathbb{E}_{x \sim Q}\left[\left(\frac{P(x)}{Q(x)}\right)^\alpha\right]$$

A mechanism $\mathcal{M}$ satisfies $(\alpha, \rho)$-RDP if for all adjacent datasets $D, D'$:

$$D_\alpha(\mathcal{M}(D) \| \mathcal{M}(D')) \leq \rho$$

**Key advantages of RDP for FL:**

1. **Tight composition:** RDP values simply add under composition: $\rho_{\text{total}}(\alpha) = \sum_t \rho_t(\alpha)$. No looseness from union bounds.

2. **Multiple orders:** By tracking $\rho$ at many orders $\alpha$ simultaneously (we use $\alpha \in \{1.25, 1.5, 2, 3, 4, 5, 8, 16, 32, 64, 128\}$), we can convert to the tightest possible $(\epsilon, \delta)$-DP guarantee via:

$$\epsilon = \min_\alpha \left[\rho_{\text{total}}(\alpha) + \frac{\log(1/\delta)}{\alpha - 1}\right]$$
...
For the subsampled Gaussian mechanism with noise $\sigma$ and sampling rate $q$:

$$\rho(\alpha) \approx \frac{\log(1 + q^2 \cdot (\exp(\alpha / \sigma^2) - 1))}{\alpha - 1}$$

Our system implements an online RDP odometer that accumulates $\rho(\alpha)$ at all tracked orders after each round, converting to $(\epsilon, \delta)$-DP on demand for budget checking.
...
3. When the server sums all masked updates, the masks cancel out: $\sum_i \text{masked}_i = \sum_i \text{update}_i$.
...
- ✅ **GOOD** channel (loss $\leq$ 2%): SA active — bandwidth is available
- ⚠️ **MODERATE** channel (loss $\leq$ 15%): SA active — acceptable overhead
...
(Rows: from GOOD, from MODERATE, from BAD)

**Stationary distribution:** By solving $\pi P = \pi$, the long-run fraction of time spent in each state is approximately $\pi = (0.51, 0.29, 0.20)$. This means roughly 51% of client-rounds experience good channels, 29% moderate, and 20% bad.
...
| Zheng et al. [7] | ❌ | ✅ | ❌ | — |
| Chen et al. [6] | ✅ | ❌ | ❌ | — |
| Liu et al. [8] (SASH) | ❌ | ✅ | ✅ | Gradient-based |
| **Ours** | **✅** | **✅** | **✅** | **Link-based** |
...
1. **📦 Data Layer:** WISDM sensor data partitioned across 30 clients via Dirichlet sampling ($\alpha = 0.5$)
...
4. **🎛️ Scheduler:** Dynamic privacy budget allocator with three adaptation factors
5. **🔐 Aggregation Layer:** FedAvg with link-aware SA toggling and RDP accounting

### 4.2 Dynamic Privacy Budget Scheduler

The scheduler is the core intellectual contribution. It allocates the per-round privacy budget $\epsilon_t$ by modulating a base allocation with three multiplicative factors:

$$\epsilon_t = \epsilon_{\text{base}}(t) \times f_{\text{channel}}(S_t) \times f_{\text{phase}}(t) \times f_{\text{urgency}}(t)$$

#### 4.2.1 Base Allocation

The starting point distributes the total budget uniformly: $\epsilon_{\text{base}} = \epsilon_{\text{total}} / T$. For $\epsilon_{\text{total}} = 10$ and $T = 50$ rounds, this gives $\epsilon_{\text{base}} = 0.2$ per round.
...
- $f_{\text{channel}}(\text{GOOD}) = 1.4$: Allocate 40% more budget when the majority of clients have good links. Updates will likely arrive, making this round valuable.
- $f_{\text{channel}}(\text{MODERATE}) = 1.0$: Neutral — no adjustment.
- $f_{\text{channel}}(\text{BAD}) = 0.6$: Reduce budget by 40% when most clients have poor links. Many updates will be lost, so spending less budget reduces waste.
...
$$f_{\text{phase}}(t) = \beta + A \cdot \cos\left(\frac{\pi t}{T}\right)$$

with base $\beta = 0.9$ and amplitude $A = 0.3$. This produces values ranging from 1.2 (early) to 0.6 (late), allocating more budget to early rounds where the model benefits most from lower noise, and conserving budget in later rounds where the model is already close to convergence.
...
$$f_{\text{urgency}}(t) = 1 + s \cdot \tanh\left(\frac{\text{deficit}}{0.1 \cdot \epsilon_{\text{total}}}\right)$$

where $\text{deficit} = \text{planned\_spending}(t) - \text{actual\_spending}(t)$ and $s = 0.5$ is a sensitivity parameter.
...
A critical implementation detail: converting the target per-round $\epsilon_t$ into a noise multiplier $\sigma$. The naive approach uses the closed-form Gaussian mechanism formula:

$$\sigma = \frac{\sqrt{2 \cdot \ln(1.25/\delta)}}{\epsilon_t}$$

However, this formula produces $\sigma$ values that are inconsistent with the RDP accountant. The $\sigma$ that achieves $\epsilon_t$ under the basic Gaussian mechanism formula does not result in $\epsilon_t$ being recorded by the RDP odometer — the two accounting frameworks give different answers for the same $\sigma$.

To maintain consistency, we calibrate $\sigma$ via binary search: for a given target $\epsilon_t$, we iteratively try $\sigma$ values and check what the RDP accountant would record for each one, finding the $\sigma$ that produces the desired RDP contribution. This ensures the scheduler's "plan" (target $\epsilon_t$ per round) matches the odometer's "measurement" (actual RDP-tracked $\epsilon$).

### 4.4 Budget Enforcement

The system enforces a hard privacy budget cap. After each round's expenditure is recorded by the RDP odometer, the system checks whether cumulative $\epsilon$ exceeds $\epsilon_{\text{total}}$. If the budget is exhausted, all subsequent rounds receive zero allocation and training terminates early, preventing privacy overspending under any circumstance. 🚫
...
3. **Multi-sensor fusion:** With 4 sensors (phone accelerometer, phone gyroscope, watch accelerometer, watch gyroscope), the total feature vector is $48 \times 4 = 192$ dimensions.
...
4. **Activity grouping:** For our experiments, we group the 18 activities into 3 categories:
   ...
   - **🍽️ Hand-Eating:** Eating soup, eating chips, eating pasta, drinking from cup, eating sandwich
...
**Total parameters:** approximately 227,000. We use LayerNorm instead of BatchNorm for compatibility with DP-SGD (BatchNorm computes statistics across samples, which conflicts with per-sample privacy). The optimizer is AdamW with cosine learning rate annealing from $10^{-3}$ to $10^{-5}$ and weight decay $10^{-4}$. Label smoothing of 0.1 is applied to the cross-entropy loss.
...
| Dirichlet $\alpha$ | 0.5 | Moderate non-IID heterogeneity |
| Total $\epsilon$ | 10.0 | At the knee of privacy-utility curve |
| $\delta$ | $10^{-4}$ | Standard failure probability |
...
- **💰 Cumulative $\epsilon$ spent:** Total privacy budget consumed (from RDP odometer)
- **📊 Communication volume:** Total bytes transmitted (model uploads + SA overhead)
- **📈 F1 score (macro/weighted)** and **Cohen's $\kappa$** for final model quality
...
We compare four budget allocation strategies, all with $\epsilon_{\text{total}} = 10$ and 50 rounds:

| Schedule | Best Test Accuracy | $\epsilon$ Spent | Description |
|----------|-------------------|---------|-------------|
| **🎛️ Dynamic (Ours)** | **75.19%** | 1.07 | Channel × Phase × Urgency |
...
The dynamic and uniform schedules spent identical $\epsilon$ (1.07), indicating that the dynamic scheduler's modulation (channel/phase/urgency adjustments) redistributes budget across rounds but does not change the total amount consumed at convergence.

### 6.2 Experiment 2: Privacy-Utility Curve

We sweep $\epsilon \in \{2, 4, 6, 8, 12, 16, 20\}$ under the dynamic schedule with noisy channels:

| $\epsilon$ | Best Test Accuracy | Regime |
|---|-------------------|--------|
...
**Analysis:** The curve exhibits the characteristic shape of the privacy-utility tradeoff: steep improvement from $\epsilon = 2$ to $\epsilon = 6$ as noise decreases from prohibitive to manageable, a knee around $\epsilon = 6$ where the slope changes markedly, and diminishing returns above $\epsilon = 12$.

This is the first privacy-utility characterization for federated activity recognition under noisy AIoT conditions. The practical implication is that $\epsilon = 6\text{--}10$ represents the "sweet spot" where meaningful privacy protection is achieved without catastrophic accuracy loss.

### 6.3 Experiment 3: Ablation Study

We evaluate the contribution of each scheduler factor by disabling them individually:

| Configuration | Best Test Accuracy | $\Delta$ from Full |
|--------------|-------------------|-------------|
| 🎛️ Full Dynamic | 75.19% | — |
| −Channel | 75.21% | +0.0pp |
| −Phase | 75.13% | −0.1pp |
| **−Urgency** | **73.62%** | **−1.6pp** |
...
| Configuration | Accuracy | $\epsilon$ Spent | Comm. (MB) |
|--------------|----------|---------|------------|
...
1. **💸 Cost of DP on reliable links:** 94.75% → 78.02% = **−16.73pp**
   - This is the privacy tax: adding $(\epsilon=10, \delta=10^{-4})$-DP with $\sigma \approx 2.2$ to every gradient reduces accuracy by nearly 17 percentage points.
...
| $\sigma$ | Our $\epsilon$ | Opacus $\epsilon$ | Difference |
|---|-------|----------|------------|
...
Our accountant consistently reports lower $\epsilon$ values (more conservative estimates). This is because we use the dominant-term approximation of the subsampled Gaussian RDP bound, while Opacus uses the full analytic bound. The practical implication is that our stated privacy guarantees are stronger than reported — the system provides better privacy protection than the $\epsilon$ values suggest.
...
2. **Moderate multipliers (1.4/1.0/0.6) produce small per-round effects.** The ±40% budget modulation translates to modest $\sigma$ variation (2.2-4.6), which is insufficient to create a measurable accuracy gap over 50 rounds.
...
Our system provides formal $(\epsilon, \delta)$-differential privacy with $\epsilon = 10.0$ and $\delta = 10^{-4}$. The guarantee is:

- **Tracked via RDP composition** with online budget enforcement
- **Conservative** — our accountant reports lower $\epsilon$ than Opacus, meaning actual privacy is stronger than stated
...
2. **📈 The privacy-utility knee is at $\epsilon \approx 6$:** Below this, accuracy degrades catastrophically. Above $\epsilon \approx 12$, gains are marginal.

3. **⏱️ Budget urgency is the most valuable scheduler factor:** The urgency-based feedback loop improves accuracy by 1.7pp over static uniform allocation by ensuring productive use of the full privacy budget.
...
4. **✂️ Adaptive gradient clipping** that restores the gradient factor's utility by tracking norms before Opacus clipping
